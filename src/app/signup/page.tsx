...
import { User as UserIcon, Lock, Mail, AlertCircle } from "lucide-react";
...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !email || !password || !captchaVerified) return;
    ...
    try {
      ...
      if (err.code === "auth/email-already-in-use") {
        setError(language === "en" ? "This email is already registered." : "هذا البريد الإلكتروني مسجل بالفعل.");
      } else {
        setError(language === "en" ? "Failed to create account. Please try again." : "فشل إنشاء الحساب. يرجى المحاولة مرة أخرى.");
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const handleSocialLogin = async (provider: "google" | "apple" | "facebook" | "github") => {
    ...
    try {
      ...
      setError(language === "en" ? "Failed to sign up with provider." : "فشل إنشاء الحساب باستخدام الحساب المحدد.");
    }
  };
      ...
      <div className="relative z-10 w-full max-w-md bg-black/75 p-8 md:p-12 rounded-2xl border border-gray-800 shadow-2xl backdrop-blur-md space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-xs text-gray-400">
            {language === "en" ? "Create your account to start watching movies & series." : "أنشئ حسابك الجديد الآن للبدء في مشاهدة آلاف الأفلام والمسلسلات."}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder={language === "en" ? "John Doe" : "عماد الفاسي"}
        ...
        <div className="relative flex items-center justify-center my-4">
          ...
          <span className="bg-[#111111] px-3 text-[11px] text-gray-500 font-semibold absolute">
            {language === "en" ? "OR REGISTER WITH" : "أو التسجيل باستخدام"}
          </span>
        </div>
